...
export default function Product() {
  const columns = [
    ...
    },
    {
      name: "Desc",
      cell: (row) => (
        <button className="text-[#2563EB]" onClick={() => descUmkm(row.desc)}>
          Detail
        </button>
      ),
    },
  ...
  const getProduct = async () => {
    try {
      const req = await axios.get("http://127.0.0.1:8000/api/umkm");
  ...
  useEffect(() => {
    const result = data.filter((item) => {
      return item.umkm.toLowerCase().match(search.toLocaleLowerCase());
    });
    setFilter(result);
  }, [search]);
